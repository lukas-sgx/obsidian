...
static const char *const help_configuration[] = {
    "Configuration:",
    ("- .obsidianrc: Create this file in your home directory or "
        "current directory"),
    "  to automatically execute commands at startup.",
    "  Use 'exec <command>' to execute shell commands.",
};

static void print_lines(const char *const *lines, int count)
{
...
void help(char *input, history_t *history)
{
    ...
    (void)input;
    (void)history;
    command_count = sizeof(help_commands) / sizeof(help_commands[0]);
    configuration_count =
        sizeof(help_configuration) / sizeof(help_configuration[0]);
    printf("Available commands :\n");
    print_lines(help_commands, command_count);
    printf("\n");
    print_lines(help_configuration, configuration_count);
    printf("\n");
}
